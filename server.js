...
function createScenePrompts(story) {
  ...
  for (let i = 0; i < SCENE_COUNT; i++) {
    ...
    prompts.push(
      ...
      `Create a visually beautiful, coherent scene with natural movement, ` +
      `cinematic camera motion, detailed environment, realistic lighting, ` +
      `high quality film look, consistent visual storytelling. ` +
...
async function generateClip(prompt, ratio, filePath) {
  ...
  const taskRequest = client.imageToVideo.create({
    model: "gen4.5",
    promptText: prompt,
    ratio: ratio,
...
async function generateFullVideo(
  ...
  try {
    ...
    job.progress = 90;
    job.message =
      "Combining your scenes into one video...";
...
app.use((req, res) => {
  res.status(404).json({
    success: false,
    message: "VEOX AI endpoint not found."
  });
});
